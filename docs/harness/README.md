...
| (2026-08-20 CI 실측 — main `f4a3c03`, run #295) | **71.37%** | 88 | **328** | **67** |

> **지금 break를 올릴 시점이다.** 실측 71.37%에 게이트가 67이라 여유가 4.37%p다.
> 위 규칙("검사를 보강할 때마다 break를 함께 올린다")대로면 **69**가 다음 칸이다.
> 올리지 않은 채로 두면 래칫이 아니라 그냥 낮은 기준이고, 이 표가 그 사실을 숨긴다.
