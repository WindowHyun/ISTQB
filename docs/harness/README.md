...
| `stryker.storage.config.json` | `storage.ts`·`useQuizStore.ts` | 62.07% | 58 | **래칫**(50 → 58) |
...
실제로 한 번 돌렸다 — 복원 분기 검사 20건을 추가하니 53.82% → **62.07%**(+8.25%p)가 되어
break를 50 → 58로 올렸다. 눈여겨볼 것은 no-coverage가 **222 → 116**으로 절반이 된 점이다:
점수 상승분의 대부분은 '검사를 더 강하게' 해서가 아니라 **유닛이 아예 지나가지 않던 코드에
처음 검사가 닿아서** 나왔다. 남은 116건이 다음 차례이고, 그쪽이 가장 값이 싸다.
