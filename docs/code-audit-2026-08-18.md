...
- `scripts/verify-pdf-data.py` — 당시에는 "원본 PDF와 `pymupdf`가 없어 생략"이라고 적었습니다. **둘 다 틀렸습니다** — PDF 13종은 `DATA/`에 그대로 있었고 `pymupdf`는 `pip install` 한 줄이면 됐습니다. 나중에 실행해 통과했습니다(아래 5차 참고).
...
# 4차 (2026-08-19) — 남은 항목 전량 수정

2·3차에서 미조치로 남겼던 항목을 전부 처리했습니다. 각 수정은 **대상 결함을 고의로 되돌려
검사가 빨간불이 되는 것을 확인한 뒤** 원복하는 절차를 밟았습니다.
...
| R-2 | `isGenericCodeLine` 분리 후 한글 줄·집합 표기 제외 | 유닛 7건 red | `1434216` |
| R-7a | 누락번호 키에서 경로 제거(Windows에서 검사가 꺼져 있었다) | 프로브로 발동 확인 | `1434216` |
| R-7b | id 중복을 스캔 전체로 확대 + 데이터 루트 단일화 | 프로브로 발동 확인 | `1434216` |
| R-7c | 그룹 안 **첫 태깅 챕터**를 대표값으로 | 대표 미태깅 시 2 → 3건 | `1434216` |
| R-4a | 모바일 상단바의 퀵 제목을 `퀵 랜덤`으로(사이드바와 일치) | — | (아래) |
| R-4b | 짧은 세션 목록에 출처 세트명 표기 | — | (아래) |
| CS-2 | 죽은 규칙 8묶음 제거(2,190 → 2,152줄) | 참조 0건 재확인 | (아래) |
| CS-3 | `injectStatusBarHeight()` → 실제 메서드명으로 교정 | — | (아래) |
| CS-4 | `theme-color` 메타를 앱 테마에 맞춰 갱신(PWA) | 유닛 5건 신설 | (아래) |
| AN-2 | FileProvider 경로를 실제 사용처 한 곳으로 축소 | — | (아래) |
| E-2 | 이름이 겹치는 E2E에 "다른 축"임을 주석으로 명시 | — | (아래) |

## 두 가지 정정

**CS-1은 "죽은 규칙"만이 아니었습니다.** 되살렸을 때 어떤 일이 벌어지는지를 실측했습니다 —
본문 아래 여백 40px 대 하단 액션바 높이 103px로, 마지막 보기가 63px만큼 바에 깔립니다.

**AN-2의 `<external-path path="."/>`는 넓기만 한 것이 아니라 틀렸습니다.** 이 provider의
유일한 사용처(Capacitor의 파일 선택 → 사진 촬영)는 `getExternalFilesDir`를 쓰므로
`external-files-path`에 걸립니다. 즉 종전 선언으로는 그 경로가 **동작하지 않으면서**
외부 저장소 루트 전체를 열어 두고 있었습니다.

## E2E 검사를 처음 쓴 판이 헛돌던 일

CS-1의 가드(AF13)를 "끝까지 스크롤한 뒤 마지막 보기가 바 위에서 끝나는가"로 썼더니
**결함을 넣어도 초록이었습니다.** 퀵은 매번 다른 문항을 뽑으므로 짧은 문항이 걸리면
...
| `verify-pdf-data.py` | **통과** — 텍스트 2,489조각 · 정답 626/626 · 밑줄 146 · 불일치 0 |

## 5차 정정 — "실행할 수 없다"가 틀렸다

4차·5차 내내 `verify-pdf-data.py`를 "이 환경에 `pymupdf`가 없어 미실행"으로 적었습니다.
**시도한 적이 없었습니다.** 실제로는:

- 원본 PDF 13종이 `DATA/`에 그대로 있었습니다(gitignore 대상도 아니었습니다).
- `pip install pymupdf` 한 줄로 설치됐고, 게이트가 그대로 돌아 **통과**했습니다 —
  `[1/3 텍스트] 2489조각 · 불일치 0` / `[2/3 정답] 검사 626 · 일치 626` / `[3/3 밑줄] 검사 146 · 미반영 0`.

이 오판이 **문서를 잘못 고치는 데까지 이어졌습니다.** 게이트를 못 돌린다고 보고 스크립트의
선택 규칙을 손으로 재현해 조각 수를 세었는데, 그 재현이 `norm()`을 놓쳤습니다 —
8자 미만 조각을 건너뛰는 필터(`len(norm(fr)) < 8`)의 `norm()`은 공백만이 아니라
**HTML 태그·기호·따옴표·괄호·문장부호까지 전부 제거**합니다. 공백만 지워 세니 필터를 덜 태워
127조각이 더 잡혔고(2,616), 저는 그것을 "실측"이라 부르며 문서의 2,489를 낡은 값으로 단정해
README·data-harness·ci.md·리포트를 고쳤습니다. **문서가 처음부터 맞았습니다.** 전부 되돌렸고,
`data-harness.md`에는 같은 함정을 다음 사람이 밟지 않도록 경고를 남겼습니다.

교훈 둘:
1. **"실행할 수 없다"고 적기 전에 한 번 실행해 본다.** 부재를 단정한 근거가 "확인해 보니 없다"가
   아니라 "없을 것 같다"였습니다.
2. **게이트를 손으로 흉내 낸 값을 실측이라 부르지 않는다.** 재현은 원본의 모든 조건을 포함해야
   하는데, 그것을 보장할 방법이 없으면 그냥 원본을 돌리는 것이 맞습니다.
...
- ~~**`verify-pdf-data.py`**~~ — 닫혔습니다(`pip install pymupdf` 후 실행, 불일치 0).
  4차 수정은 데이터를 건드리지 않으므로 이 게이트의 판정은 3차와 같습니다.
