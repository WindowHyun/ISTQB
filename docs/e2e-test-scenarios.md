...
| Q-4 | 퀵의 이동 수단은 **‹ › 뿐**이다 — 데스크톱의 '⤢ 문항 이동', 모바일 하단바의 점프 핀, 번호 팔레트가 모두 DOM에서 빠진다(CSS 숨김이 아니다: 키보드·스크린리더에는 남기 때문). 연습에서는 셋 다 있는 것을 같은 검사 안에서 확인해 '퀵에서만 빠짐'을 못 박고, 남은 ‹ › 가 실제로 앞뒤로 움직이는지도 본다 | react-quick-ux |
