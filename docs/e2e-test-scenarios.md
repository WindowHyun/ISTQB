...
| `react-quick` | 27 | **퀵**(무한) 즉시 피드백·다음 문제·점수판·무기록·세트 잠금·**커서 규칙**(한 바퀴 완료·확정 없이는 못 넘김·화살표 차단)·**폐지 사양 광고 금지**(게이트 문구) |
...
| `react-quick-resilience` | 1 | 퀵의 부분 로드 실패 — 세트 하나를 못 읽어도 나머지로 출제한다 |
...
Pixel 7 디바이스 프로파일 + WebView UA + `MainActivity`의 안전영역 주입 모사. 기능 12(AF1~AF12): 상단바·드로어·하단 액션바의 상태바/제스처바 회피, 터치 풀이·채점, 웹뷰 재시작 복원, 가로 넘침 금지, 가로 모드 2종, **AF11 퀵 컨트롤의 제스처바 회피·44px 터치 타깃**, **AF12 '다음 문제'의 제스처바 회피 + 웹뷰 재시작 후 퀵 진행 유지**. 비기능 8(ANF1~ANF8): 모바일 로드·렌더·이동 성능, 드로어 개폐·연타 스트레스, 채점 응답, 재시작 5회 내구성, DOM/힙 예산.
...
- 테스트 효력 확인: 새로 추가한 검사는 대상 결함을 일부러 되돌려 **실패하는 것을 본 뒤** 원복하는 절차를 거친다 (예: 퀵 진행 집계 검사는 커서 파생을 상수로 바꾸면 '진행'과 '정답+오답'이 어긋나 실패한다).
...
| ST-* | 상태 전이 전수(S0 게이트/S1 연습/S2E 시험 3단계/S4 오답 × 전이) | react-transition |
