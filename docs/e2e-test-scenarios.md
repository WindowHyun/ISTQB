...
## Safari/WebKit 60 — `npm run test:webkit`

Desktop Safari 프로파일로 **엔진 계층을 지나는 핵심 경로만** 다시 태운다(IndexedDB 트랜잭션·Blob 다운로드·서비스워커·Date 파싱). 전 스펙 재실행은 e2e 잡을 두 배로 만들면서 대부분 중복이라 하지 않는다.

이 프로젝트만 타임아웃이 **180초**다. WebKit에서 **문항 하나를 렌더할 때마다 메인 스레드가 1~1.4초 블록되기** 때문에(실측: 빈 페이지 21프레임/400ms · 게이트 14 · 문항 2, Chromium은 같은 지점에서 25), 보기→다음을 10~70회 반복하는 긴 루프가 기본 30초 안에 끝나지 못한다. 종전에는 그 5건을 범위 밖에 뒀지만, 원인이 "테스트가 틀렸다"가 아니라 "앱이 Safari에서 느리다"로 밝혀져 제외를 걷고 타임아웃만 늘렸다 — 느린 것을 안 보이게 만드는 대신 통과 여부를 계속 확인한다. `react-webkit-motion`이 그 진단을 담고 있으며, 서비스워커 precache 가설은 같은 파일의 진단이 반증했다(캐시가 이미 찬 상태에서도, SW를 지운 뒤에도 동일).
