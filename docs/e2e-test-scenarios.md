...
| `react-study-ux` | 22 | 이어풀기 배너·제출 전 검토·오답노트 재설계(세트 선택)·시험 모드 유지/재응시·이어풀기·새로풀기 선택·결과 nowrap·피드백 aria-live |

> 아래는 초기 핵심 70개의 상세 G·W·T이며, 이후 엣지·확장 181개는 위 인덱스의 각 스펙 파일에 동일한 G·W·T 구조로 구현되어 있습니다.
...
- 로컬 검증: 설치된 chromium headless 기준 **251/251 통과**. CI는 자체 브라우저로 동일 실행.
