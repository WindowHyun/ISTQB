...
test.describe("코드리뷰 수정 회귀 — 오답 목록 보존·가드 이동 초기화", () => {
  // ⚠ 이름이 비슷한 다른 축이 있다. 여기서 보는 것은 **시험 키 ↔ 랜덤 키**다 — 그 둘은
  // 원래 채점 키가 달라(gradeKeyFor에 mode가 들어간다) 서로 덮어쓸 일이 없다.
  // 챕터 미니 시험이 세트 전체 랜덤과 **같은 키를 공유하던** 문제(오답 모드 출제 대상이
  // 사라지던 결함)는 다른 축이고, 이 검사는 그 수정 전후 모두 통과한다.
  // 그쪽 회귀는 reviewKeyFor/isReviewKeyOf의 유닛이 맡는다 — 이름만 보고 "이미 덮여
  // 있다"고 판단하지 말 것.
