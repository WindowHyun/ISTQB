(async () => {
    ...
    return {
      ...
      questions: Array.isArray(payload?.questions)
        ? payload.questions.map((question) => ({
            ...
            legacySetId:
              meta.legacySetId || catalogItem.legacySetId || meta.setId,
    ...
    try {
      ...
      const loadedSets = await Promise.all(
        items.map(async (item) => ({
          item,
          payload: await fetchQuestionJson(
      ...
      Object.entries(productData).forEach(([product, value]) => {
        if (value.sets.length === 0) {
          questionDataErrors[product] =
            `${productLabels[product]} question data is empty.`;
        }
      });
    } catch (error) {
  ...
  const mobileProgressBadge = document.querySelector("#mobileProgressBadge");
  ...
  const figureModalCloseBtn = document.querySelector("#figureModalCloseBtn");
  ...
  const backupImportCloseBtn = document.querySelector("#backupImportCloseBtn");
  ...
  function currentSet() {
    return (
      ...
      data.sets[0] || { ...emptySet, title: productLabels[activeProduct] }
    );
  }

  function validSetId(setId) {
    if (!data.sets.length) return "";
    return data.sets.some((set) => set.id === setId) ? setId : data.sets[0].id;
  }
  ...
  function randomQuestions() {
    const targetCount = Math.min(40, allQuestions().length);
    if (
      state.randomRefs.length !== targetCount ||
      state.randomRefs.length === 0
    ) {
      generateRandomRefs();
    }
  ...
  function updateProductChrome() {
    if (productSubtitle)
      productSubtitle.textContent = productLabels[activeProduct];
    if (productTitle) {
  ...
  function closeModal(modal, onClose) {
    ...
    if (
      ![figureModal, wrongNoteModal, backupImportModal, consoleLogModal].some(
  ...
  function renderCstsPage() {
    ...
    cstsState.index = Math.max(
      0,
      Math.min(cstsState.index, questions.length - 1),
    );
  ...
  function saveBackupWithAndroid(fileName, payload) {
    ...
    try {
      ...
      throw new Error(
        error.message || "Android 다운로드 폴더에 백업을 저장하지 못했습니다.",
      );
    }
  }
  ...
  function saveTextWithAndroid(fileName, payload) {
    ...
    try {
      ...
      throw new Error(
        error.message || "Android 다운로드 폴더에 파일을 저장하지 못했습니다.",
      );
    }
  }
  ...
  async function exportBackup() {
    ...
    try {
      if (
        ...
        if (navigator.canShare({ files: [file] })) {
          ...
          setBackupStatus(backupExportMessage(fileName, "share"), "success");
          return;
        }
      }
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        setBackupStatus(backupExportMessage(fileName, "clipboard"), "success");
      } catch {
        setBackupStatus(
    ...
    return value
      ...
      .map((history) => {
        const mode = ["exam", "random", "review"].includes(history.mode)
          ? history.mode
          : "exam";
        const setId = mode === "random" ? "random" : validSetId(history.setId);
  ...
  function extractBackupSnapshot(payload) {
    ...
    const snapshot = isPlainObject(payload.snapshot)
      ? payload.snapshot
      : payload;
    if (!isPlainObject(snapshot.answers) || !isPlainObject(snapshot.uiState)) {
  ...
  function applyBackupSnapshot(snapshot) {
    ...
    state.mode = validMode(uiState.mode);
    state.index =
      Number.isInteger(uiState.index) && uiState.index >= 0 ? uiState.index : 0;
  ...
  function resetAnswersFor(mode) {
    ...
    if (mode === "random") {
      state.histories = state.histories.filter((h) => h.mode !== "random");
    } else {
      state.histories = state.histories.filter(
        (h) => !(h.mode === mode && h.setId === state.setId),
      );
    }
  }
  ...
  function renderFigure(question) {
    ...
    zoomBtn.addEventListener("click", () => openFigureModal(src, image.alt));
  ...
  function appendPlainLine(target, text, options = {}) {
    ...
    splitDenseQuestionText(text)
      .flatMap(splitFormulaIntro)
      .forEach((part) => {
        const lineNode = document.createElement("span");
        lineNode.className = "text-line";
  ...
  function splitFormulaIntro(text) {
    ...
    const index = value.search(
      /(?:\b(?:E|A|AA|EE)|[𝐸𝐴]{1,2})\s*\([^)]*\)\s*[=＝]/,
    );
    if (index <= 24 || !/[=＝]/.test(value.slice(index))) return [value];
    return [value.slice(0, index).trim(), value.slice(index).trim()].filter(
      Boolean,
    );
  }
  ...
  function isQuestionPromptLine(text) {
    ...
    return /[?？]$/.test(value);
    return (
      /[?？]$/.test(value) || /^(다음 중|어느|어떤|가장|최소|최대)/.test(value)
    );
  }
  ...
  function render() {
    ...
    gradeBtn.textContent =
      state.mode === "review" && isReviewRetake() ? "오답 재채점" : "채점하기";
    ...
    const shouldResetQuestionScroll = questionKey !== lastRenderedQuestionKey;
  ...
  function renderNav(questions) {
    ...
    questions.forEach((question, index) => {
      ...
      button.classList.toggle("unanswered", selectedFor(question).length === 0);
      button.addEventListener("click", () => {
        feedbackExpanded = false;
        state.index = index;
        render();
  ...
  function historyWrongNoteItems(history) {
    ...
    questions.forEach((question) => {
      const ansKey = answerKey(
        question,
        history.mode === "random" ? "random" : "exam",
      );
      const selected = history.answers[ansKey] || [];
      const isCor =
        question.type === "short_answer"
          ? normalizeTextAnswer(selected[0]) ===
            normalizeTextAnswer(question.answer[0])
  ...
  function hasWrongNoteItems() {
    return state.histories.some(
      (history) => historyWrongNoteItems(history).length > 0,
    );
  }
  ...
  function openWrongNote() {
    ...
    if (!validHistory || historyWrongNoteItems(validHistory).length === 0) {
      const historiesWithErrors = [...state.histories]
        .reverse()
        .filter((h) => historyWrongNoteItems(h).length > 0);
  ...
  function renderWrongNote() {
    ...
    const createFilterBtn = (value, text) => {
      ...
      if (wrongNoteFilter === value) {
        btn.className = "primary wrong-note-filter-btn";
      } else {
        btn.className = "subtle wrong-note-filter-btn";
      }
    ...
    historiesWithErrors.forEach((history, index) => {
      ...
      const setTitle =
        history.mode === "random"
          ? "랜덤"
          : data.sets.find((s) => s.id === history.setId)?.title ||
            history.setId;
      const count = historyWrongNoteItems(history).length;
      filterRow.appendChild(
        createFilterBtn(
          history.id,
          `기록 ${historiesWithErrors.length - index}: ${setTitle} (${timeStr})`,
        ),
      );
    });
    ...
    const currentHistory = state.histories.find(
      (h) => h.id === wrongNoteFilter,
    );
    ...
    if (items.length === 0) {
      ...
      items.forEach((question) => {
        ...
        const text = document.createElement("p");
        text.className = "wrong-note-text";
        text.textContent = stripPdfNoise(
          plainQuestionText(question.stem),
        ).split("\n")[0];
        ...
        go.addEventListener("click", () => {
          ...
          const index = questions.findIndex(
            (item) =>
              (item.setId || state.setId) === (question.setId || state.setId) &&
  ...
  function resetCurrentSet(skipConfirm = false) {
    ...
    if (state.mode === "exam" || state.mode === "review") setExamGraded(false);
  ...
  gradeBtn.addEventListener("click", () => {
    ...
    if (gradedNow) {
      ...
      const targetQuestions =
        state.mode === "random" ? randomQuestions() : currentSet().questions;
      const historyAnswers = {};
      targetQuestions.forEach((question) => {
        const key = answerKey(question, targetMode);
        if (state.answers[key]) historyAnswers[key] = state.answers[key];
      });
  ...
  retryWrongBtn.addEventListener("click", () => {
    ...
    state.reviewIds[state.setId] = missed.map((question) => question.number);
    missed.forEach(
  ...
  cstsPrevBtn?.addEventListener("click", () => {
    ...
    if (questions.length)
      cstsState.index = Math.min(cstsState.index, questions.length - 1);
    renderCstsPage();
  });
  ...
  settingsPanelToggleBtn.addEventListener("click", (event) => {
    const isOpen =
      settingsPanelToggleBtn.getAttribute("aria-expanded") === "true";
  ...
  document.addEventListener("click", (event) => {
    if (
      ...
      !settingsPanelToggleBtn.contains(event.target)
    ) {
      settingsPanelToggleBtn.setAttribute("aria-expanded", "false");
