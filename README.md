...
> **한 줄 요약** — 단순 기능 구현을 넘어 **테스트 자동화(유닛 141 + E2E 294) · 데이터 정합성 검증 ·
...
- [테스트 전략](#테스트-전략)
- [테스트 자동화 상세 (E2E 294)](#테스트-자동화-상세-e2e-294)
...
- **테스트 자동화 435개** — Vitest 유닛 141 + Playwright **E2E 294**(스모크·모드·문항유형·네비·설정·영속성·엣지·반응형·접근성·**상태 전이 전수**·**대용량 import**·**표/그림 문항**·**라이트박스/콘솔**·**저장 불가 환경**). 전체 시나리오: [`docs/e2e-test-scenarios.md`](docs/e2e-test-scenarios.md).
- **살충제 패러독스 대응 3계층** — **속성 기반 테스트 21**(fast-check: 매 실행 새 입력 생성) + **뮤테이션 테스트**(Stryker, 채점·통계 핵심 로직 스코어 **93.5%**, CI break 85 게이트) + **시드 랜덤 스모크 E2E**(매일 다른 세트·답 조합, 원본 JSON을 독립 오라클로 사용·시드 로그로 재현 가능).
...
|---------|----------------------|------|
| 테스트 자동화 | Playwright **E2E 294개** + Vitest **유닛 141개**(속성 기반 21 포함) 작성·CI 연동 | `e2e/`, `src/**/*.test.ts` |
...
```
        ▲  E2E (Playwright) — 294개: 사용자 플로우·엣지·크로스뷰포트·A11y·시드 랜덤 스모크
       ───
      ─────  통합/렌더 — jsdom 렌더 테스트(파서·RichText)
     ───────  유닛 (Vitest) — 141개: 정답판정·파서·컷스코어·챕터/회차 통계·저장·타이머·토스트·store 액션·store↔storage 연동·속성 기반 21
    ─────────  데이터 검증 — verify + PDF 전수 대조(정답·이미지·스키마, 626문항)
```

- **계층별 역할 분리**: 로직은 유닛, 화면 흐름은 E2E, 데이터는 정합성 스크립트로 검증.
- **살충제 패러독스 대응**: 고정 케이스가 무뎌지는 것을 막기 위해 ①속성 기반(fast-check — 매 실행 새 입력) ②뮤테이션(Stryker — 테스트의 결함 검출력 실측, 스코어 93.5%·CI break 85) ③시드 랜덤 스모크 E2E(매일 다른 조합, 데이터(JSON)를 독립 오라클로 사용, `SMOKE_SEED`로 재현)를 병행.
...
- GitHub Actions **10 job**(push/PR 병렬):
  - **기능·품질 7** — `lint`(ESLint+tsc) · `verify(데이터)` · `unit`(+커버리지 임계값 게이트) · `mutation`(Stryker 뮤테이션 스코어 break 85 게이트) · `build`(+번들 크기 예산) · `e2e`(기능 294) · `nonfunctional`(성능·부하·메모리·타이머·오프라인·데이터 내구성·장기 스케일 12, CI 완화 예산).
...
- 자동화 테스트: **유닛 141 + E2E 294 = 435** (전 문항 626 렌더 스윕 별도).
- 뮤테이션 스코어(채점·통계 핵심 4개 유틸): **93.5%**(246개 뮤턴트 중 230 kill) — CI break 85 게이트로 테스트 효력 저하를 차단.
...
flowchart LR
  ...
  subgraph 품질["품질 게이트 (GitHub Actions · 10 job)"]
    CI["기능·품질: lint · verify · unit(141) · mutation(93.5%) · build · e2e(294) · nonfunctional(12)<br/>보안: audit · secrets · codeql"]
  end
  REACT -.검증.-> CI
  SRC -.정합성 verify.-> CI
```
