...
![tests](https://img.shields.io/badge/tests-51%20unit%20%2B%20229%20e2e-brightgreen)
...
> **한 줄 요약** — 단순 기능 구현을 넘어 **테스트 자동화(유닛 51 + E2E 229) · 데이터 정합성 검증 ·
...
- [테스트 전략](#테스트-전략)
- [테스트 자동화 상세 (E2E 229)](#테스트-자동화-상세-e2e-229)
...
- **테스트 자동화 280개** — Vitest 유닛 51 + Playwright **E2E 229**(스모크·모드·문항유형·네비·설정·영속성·엣지·반응형·접근성·**대용량 import**·**표/그림 문항**·**라이트박스/콘솔**). 전체 시나리오: [`docs/e2e-test-scenarios.md`](docs/e2e-test-scenarios.md).
...
|---------|----------------------|------|
| 테스트 자동화 | Playwright **E2E 229개** + Vitest **유닛 51개** 작성·CI 연동 | `e2e/`, `src/**/*.test.ts` |
...
```
        ▲  E2E (Playwright) — 229개: 사용자 플로우·엣지·크로스뷰포트·A11y
       ───
...
- 자동화 테스트: **유닛 51 + E2E 229 = 280** (전 문항 626 렌더 스윕 별도).
...
flowchart LR
  ...
  subgraph 품질["품질 게이트 (GitHub Actions)"]
    CI["lint · verify · unit(51) · build · e2e(229)"]
  end
  REACT -.검증.-> CI
  SRC -.정합성 verify.-> CI
```
