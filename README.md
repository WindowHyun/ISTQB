...
- **살충제 패러독스 대응 4계층** — **속성 기반 테스트**(fast-check: 매 실행 새 입력 생성) + **뮤테이션 테스트**(Stryker, 채점·통계·저장키 핵심 8개 유틸 스코어 **93.9%**, CI break 85 게이트) + **시드 랜덤 스모크 E2E**(매일 다른 세트·답 조합, 원본 JSON을 독립 오라클로 사용·시드 로그로 재현) + **몽키 테스트 3시드**(무작위 120회 조작 후 불변식·JS 오류 검사).
...
- **PWA 새 버전 갱신** — 새 버전(서비스워커)을 감지하면 **어디에 있느냐로 처리가 갈린다.**
  - **제품 선택 게이트에서는 묻지 않고 바로 적용한다.** 배너를 한 번 닫으면 그 세션에서는 다시 뜨지 않아, 누르지 않는 사용자는 계속 옛 코드로 돈다. 앱을 열 때마다 게이트를 지나므로 그 자리에서 적용하면 대부분은 아무것도 누르지 않아도 다음 실행에 최신이 된다. 게이트는 아직 아무것도 시작하지 않은 화면이라 리로드로 잃을 상태가 없다.
  - **풀이 중에는 자동 적용하지 않는다** — 하단 배너로 알리고 1탭으로 갱신. 시험에는 제한시간이 있어 경고 없는 리로드는 그 자체로 사고다.
  - 확인 시점은 앱 복귀·창 포커스·네트워크 복구·15분 주기(최소 간격 1분) · 진입 시 항상 최초 화면 복귀(bfcache 포함) · 레거시 SW 자가 해제 tombstone
...
- 문서 전체 색인(하네스·시나리오·배포·점검 기록): [`docs/README.md`](docs/README.md).
...
- 뮤테이션 스코어(채점·통계·저장키·모드규칙·세션 파생 8개 유틸): **93.91%**(755개 뮤턴트 중 709 검출) — CI break 85 게이트로 테스트 효력 저하를 차단. `sessionDerive`는 편입 전 단독 96.99%를 먼저 재서 평균을 끌어내리지 않는 것을 확인하고 넣었다(93.25 → 93.91%). 영속화·상태 계층은 별도 게이트다(**71.37%**, break 68 래칫 — 이유는 `docs/harness/README.md`).
