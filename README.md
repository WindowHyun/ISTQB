...
![tests](https://img.shields.io/badge/tests-486%20unit%20%2B%20442%20e2e-brightgreen)
...
> **한 줄 요약** — 단순 기능 구현을 넘어 **테스트 자동화(유닛 486 + E2E 442) · 데이터 정합성 검증 ·
...
- [테스트 전략](#테스트-전략)
- [테스트 자동화 상세 (E2E 442)](#테스트-자동화-상세-e2e-442)
...
- **테스트 자동화 928개** — Vitest 유닛 486 + Playwright **E2E 442**(기능 409 · 비기능 13 · **APK/WebView 20**) + **Safari/WebKit 99**(엔진 계층 재실행). 스모크·모드·문항유형·네비·설정·영속성·엣지·반응형·접근성·**5모드 전이 25칸 전수**·**대용량 import**·**표/그림 문항**·**라이트박스/콘솔**·**저장 불가 환경**·**오프라인 출제**. 전체 시나리오: [`docs/e2e-test-scenarios.md`](docs/e2e-test-scenarios.md).
...
|---------|----------------------|------|
| 테스트 자동화 | Playwright **E2E 442개**(기능·비기능·APK) + **WebKit 99개**(Safari 엔진 계층) + Vitest **유닛 486개**(속성 기반 22 포함) 작성·CI 연동 | `e2e/`, `src/**/*.test.ts` |
...
```
        ▲  E2E (Playwright) — 442개: 사용자 플로우·엣지·크로스뷰포트·A11y(axe)·시드 랜덤 스모크·몽키·APK WebView·Safari
       ───
...
- 자동화 테스트: **유닛 486 + E2E 442 = 928** (E2E 내역: 기능 409 · 비기능 13 · APK/WebView 20. 별도로 Safari/WebKit 99개가 엔진 계층을 재실행한다. 전 문항 626 렌더 스윕 포함).
...
npm run test:webkit # Safari/WebKit E2E — 엔진 계층 핵심 경로 (99개)
...
- **향후**: 모바일 Safari(iPhone) 검증 확대, **Safari 렌더 비용 개선** — WebKit에서 문항 하나를 그릴 때마다 메인 스레드가 1~1.4초 블록되는 것을 실측했다(원인 규명 완료, 제외 4건은 타임아웃 상향으로 게이트 복귀). 무엇이 그 시간을 쓰는지 좁혀 실제로 줄이는 것이 남았다, 컴포넌트 단위 테스트로 커버리지 게이트 범위 확대, Lighthouse 점수 정량화, 데이터 추출 파이프라인 자동화.
