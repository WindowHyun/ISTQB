...
![tests](https://img.shields.io/badge/tests-unit%20%2B%20e2e%20(CI%20gated)-brightgreen)
...
> **한 줄 요약** — 단순 기능 구현을 넘어 **테스트 자동화(유닛 + E2E 4개 프로젝트) · 데이터 정합성 검증 ·
...
- **살충제 패러독스 대응 4계층** — **속성 기반 테스트**(fast-check: 매 실행 새 입력 생성) + **뮤테이션 테스트**(Stryker, 채점·통계·저장키 핵심 7개 유틸 스코어 **92.44%**, CI break 85 게이트) + **시드 랜덤 스모크 E2E**(매일 다른 세트·답 조합, 원본 JSON을 독립 오라클로 사용·시드 로그로 재현) + **몽키 테스트 3시드**(무작위 120회 조작 후 불변식·JS 오류 검사).
- **PDF ↔ 데이터 전수 정합성 검증(상시 CI 게이트)** — 626문항을 **원본 PDF 13종에서 독립 추출해 텍스트 2,616조각·정답 626문항·밑줄 강조 146곳을 전수 대조**하는 게이트(`scripts/verify-pdf-data.py`, CI `pdf-data` job)를 상시 운영. 배포 전 6축 검수(텍스트·정답·밑줄·강조·이미지/그래프·표)로 표 구조 손실·그림 누락·밑줄 144곳 누락을 발견·교정해 현재 불일치 0. 더해 `npm run verify`로 정답·이미지·스키마 자동 점검 + 전 문항 렌더 스윕(HTTP 404·예외·깨진 이미지 0).
...
|---------|----------------------|------|
| 테스트 자동화 | Playwright **E2E**(기능·비기능·APK 4개 프로젝트) + Vitest **유닛**(속성 기반 포함) 작성·CI 연동 | `e2e/`, `src/**/*.test.ts` |
...
| 탐색적 테스트 | 전 문항 626 렌더/리소스 스윕(HTTP 404·예외·깨진 이미지 0) | Playwright 스윕 |

## 주요 기능

- ISTQB / CSTS 자격증 선택 → 세트 선택 → 풀이로 이어지는 진입 흐름
- **연습 · 시험 · 퀵 · 오답** 4가지 모드 + 학습 통계에서만 들어가는 **챕터 미니 시험**
  - 모드 세그먼트에는 4개만 둔다. 종전에 있던 **랜덤** 탭은 뺐다 — 세트 안 무작위(랜덤)와 전 세트 무작위(퀵)를 나란히 두면 무엇이 다른지 설명할 수 없어 퀵에 흡수했다. 랜덤 **모드 자체는 살아 있고**, 이제 통계의 **챕터 미니 시험**(그 챕터 최대 10문항)이 유일한 진입로다.
- 연습·오답 모드: 답 선택 후 **즉시** 정답/해설 확인 / 시험·미니 시험·퀵: **채점 후** 결과 확인
- **퀵 랜덤** — 제품(ISTQB/CSTS)의 **전 세트를 섞어** 제한시간 없이 푸는 짧은 세션. **문항 수를 고르지 않습니다** — 끝을 정해 두지 않고 원하는 만큼 풀다 그만두면 됩니다. 서답형도 섞여 나오되 **앞에서 어디를 끊어 봐도 30%를 넘지 않습니다**(총량이 아니라 접두 성질이라, 몇 문항만 풀고 그만둬도 서답형이 몰리지 않습니다). 여러 세트에 재수록된 동일 문항은 **46그룹 96문항**을 빌드 타임에 식별해 한 회차에 중복 출제되지 않게 하고, 재수록본끼리 챕터명이 갈리는 3문항은 대표 챕터로 통일해 집계합니다.
  - **한 문항씩 채점합니다** — 답을 고르면 정답이 바로 보이지 않고, `채점하기`를 눌러야 그 문항의 정답·해설이 열립니다. 채점하는 순간 점수판·챕터 통계·오답 목록에 반영되고, 같은 자리 버튼이 `다음 문제 ›`로 바뀝니다. 세션을 마감하는 채점과 결과 요약 모달은 이 모드에 없습니다(끝을 정해 두지 않는 모드라 마감할 것이 없습니다).
  - **앞으로는 채점한 뒤에만 갑니다** — 번호 팔레트·'문항 이동'(점프)에 더해 **앞으로 가는 화살표(›)도 없습니다**(← 키도 같은 규칙). 채점하지 않은 문항을 미리 넘겨보면 그 문항은 집계에도 남지 않은 채 지나가기 때문입니다. 뒤로(‹)는 열려 있어 이미 채점한 문항을 다시 볼 수 있습니다. 진행 현황은 문제 헤더의 **점수판**(진행·정답·오답·연속)이 맡습니다.
  - **회차 기록을 남기지 않습니다** — 응시 횟수·평균 정답률·타임라인·짧은 세션 목록 어디에도 잡히지 않고 IndexedDB에도 저장되지 않습니다(세트를 다 푼 기록이 아니므로 실전 통계를 흐리지 않게).
  - 대신 **챕터별 약점 분석에는 기여**하고, 오답은 오답노트 맨 위 **퀵 전용 임시 목록**(출처 세트명 표기)에 실립니다. 줄마다 **`오답 보기`**가 있어 누르면 노트가 닫히고 그 문항의 지문·보기·해설이 **본문 화면**에 펼쳐집니다(팝업 아님) — `← 오답 노트`·`풀이로 돌아가기`로 되돌아갑니다. 이 목록은 채점 후 **24시간이 지나면 자동으로 사라지고**, `이력 비우기`로도 함께 지워집니다.
...
- 앱을 껐다 켜도 풀이 상태·모드 복원(localStorage / IndexedDB), **재접속·세트 변경 시 시험 답안이 있으면 “이어풀기 / 새로 풀기” 선택** (미니 시험도 추첨 결과를 저장해 **새로고침하면 같은 문항으로 이어 풉니다** — 세트를 바꾸려 하면 진행 중인 답안이 있을 때 확인을 받습니다)
- **시험 채점 후 재응시** — 결과 모달의 **"다시 풀기"** 버튼 또는 활성 모드 탭 재클릭으로 원클릭 재응시(다른 모드 왕복·재접속 경로도 유지)
- 풀이 기록 JSON **내보내기 / 가져오기**
- **자격증별 합격 컷스코어** — ISTQB 65%(40문항 기준 26) / CSTS 검정방법별 배점 합산 **75%**(4지선다·서답형 1.5점, 진위형 1.0점 — 세트마다 만점이 달라 “N점 만점 기준 M점”을 그 세트에서 산출), 채점 후 합/불 결과 요약(값 줄바꿈 없이 한 줄 표기)
...
- **PWA 새 버전 업데이트 배너** — 새 버전(서비스워커) 감지 시 하단 배너로 알리고 1탭으로 갱신 · 확인 시점은 앱 복귀·창 포커스·네트워크 복구·15분 주기(최소 간격 1분) · 진입 시 항상 최초 화면 복귀(bfcache 포함) · 레거시 SW 자가 해제 tombstone
...
```
        ▲  E2E (Playwright) — 사용자 플로우·엣지·크로스뷰포트·A11y(axe)·시드 랜덤 스모크·몽키·APK WebView
       ───
      ─────  통합/렌더 — jsdom 렌더 테스트(파서·RichText)
     ───────  유닛 (Vitest) — 정답판정·파서·컷스코어·챕터/회차 통계·저장·타이머·토스트·store 액션·store↔storage 연동·추첨 분포·속성 기반
    ─────────  데이터 검증 — verify + PDF 전수 대조 상시 게이트(텍스트·정답 626·밑줄)
```

- **계층별 역할 분리**: 로직은 유닛, 화면 흐름은 E2E, 데이터는 정합성 스크립트로 검증.
- **살충제 패러독스 대응**: 고정 케이스가 무뎌지는 것을 막기 위해 ①속성 기반(fast-check — 매 실행 새 입력) ②뮤테이션(Stryker — 테스트의 결함 검출력 실측, 스코어 92.44%·CI break 85) ③시드 랜덤 스모크 E2E(매일 다른 조합, 데이터(JSON)를 독립 오라클로 사용, `SMOKE_SEED`로 재현) ④몽키(시드 3개 × 무작위 120회 조작)를 병행.
...
- **분석**: `.code-block`이 `overflow-x: auto`인데 포커스를 받을 수 없어 **키보드·스크린리더 사용자는 가로로 잘린 코드를 볼 수 없었다**(WCAG 2.1.1). 퀵만의 문제가 아니라 코드 블록이 실린 문항 **21개** 전부에 해당하는 기존 결함으로, 기존 axe 스캔이 각 세트 1번 문항에 머물러 한 번도 밟지 않았을 뿐이었다.
...
- 자동화 테스트: **유닛 761 + E2E 445 = 1,206** (E2E 내역: 기능 `react` 411 · 비기능 13 · APK/WebView 21 = `apk` 13 + `apk-nf` 8. 전 문항 626 렌더 스윕 포함). 유닛에는 속성 기반 21건이 들어 있다.
- **개수는 이 문단에만 적는다.** 종전에는 배지·요약·피라미드 등 여러 곳에 박아 뒀는데, 스위트가 바뀔 때마다 전부 어긋나 갱신 부채만 남았다(`AGENTS.md`·`docs/e2e-test-scenarios.md`도 같은 이유로 개수를 싣지 않는다). 이 절의 수치는 **2026-08-19 실측**이고, **정본은 CI 로그**다 — 판단 근거로 쓰지 않는다.
- 뮤테이션 스코어(채점·통계·저장키·모드규칙 7개 유틸): **92.44%**(622개 뮤턴트 중 575 kill) — CI break 85 게이트로 테스트 효력 저하를 차단. 영속화·상태 계층은 별도 게이트다(**69.92%**, break 67 래칫 — 이유는 `docs/harness/README.md`).
- 유닛 커버리지(로직 계층 `store`·`utils`·`hooks`): **Stmts 80.31% · Branch 76.07% · Funcs 76.54% · Lines 81.81%**. CI에서 임계값(stmt 78·branch 75·func 75·line 79) 게이트로 회귀 차단 — 실측보다 낮게 잡은 **래칫**이라 올릴 때마다 임계값도 함께 올린다. ⚠️ **branch 여유는 1.07%p뿐**(76.07 vs 게이트 75)이라 분기 하나만 새로 열려도 게이트가 걸린다 — 로직에 조건을 추가할 때는 그 분기의 테스트를 같은 커밋에 넣는다. 컴포넌트·앱 셸은 뷰 계층이라 E2E가 담당한다.
- 데이터 무결성: 626문항 정답·이미지·스키마 `verify` 통과 + **원본 PDF 대조 게이트**(텍스트 2,616조각·정답 626/626·밑줄 146/146 — 불일치 0), 전수 스윕 결과 **HTTP 404·예외·깨진 이미지 0**.
- 번들(gzip 기준): JS **126.2KB / 예산 140KB(90%)** · CSS **9.9KB / 예산 12KB(83%)**. CI `build` job의 `npm run size`로 회귀 감시. PWA precache 110 엔트리(문항 JSON 전 세트 포함 — 오프라인 퀵 출제가 여기 의존).
...
flowchart LR
  ...
  subgraph 품질["품질 게이트 (GitHub Actions · 14 job)"]
    CI["기능·품질: lint · verify · pdf-data(PDF 대조) · unit · mutation · mutation-storage · build · android-build · e2e · nonfunctional · apk<br/>보안: audit · secrets · codeql"]
  end
  REACT -.검증.-> CI
  SRC -.정합성 verify.-> CI
```
...
npm run test:e2e    # Playwright React 기능 E2E
npm run test:nf     # 비기능 E2E — 성능·부하·메모리·오프라인
...
실제로 사용자에게 내보내는 절차(Go/No-Go 체크리스트 · 버전·태그 · 웹/APK 배포 · 실기기 스모크 · **데이터를 고쳐 내보낼 때** · 롤백 · 자주 틀리는 것): [`docs/release-playbook.md`](docs/release-playbook.md).
...
- **향후**: **Safari 렌더 비용 개선** — WebKit에서 문항 하나를 그릴 때마다 메인 스레드가 1~1.4초 블록되는 것을 실측했다(원인 규명 완료 · 실측은 [`docs/harness/ui-render-harness.md`](docs/harness/ui-render-harness.md)에 보존, 자동 Safari 게이트는 투자 대비 효과로 제거하고 실기기 수동 확인으로 대체). 무엇이 그 시간을 쓰는지 좁혀 실제로 줄이는 것이 남았다, 컴포넌트 단위 테스트로 커버리지 게이트 범위 확대, Lighthouse 점수 정량화, 데이터 추출 파이프라인 자동화.
