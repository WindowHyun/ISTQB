...
- 뮤테이션 스코어(채점·통계·저장키·모드규칙·세션 파생 8개 유틸): **94.30%**(755개 뮤턴트 중 712 검출, 로컬 실측) — CI break 85 게이트로 테스트 효력 저하를 차단. `sessionDerive`는 편입 전 단독 96.99%를 먼저 재서 평균을 끌어내리지 않는 것을 확인하고 넣었다(7파일 93.25% → 8파일 94.30%). 영속화·상태 계층은 별도 게이트다(**71.37%**, break 68 래칫 — 이유는 `docs/harness/README.md`).
