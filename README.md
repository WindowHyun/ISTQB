...
![tests](https://img.shields.io/badge/tests-151%20unit%20%2B%20305%20e2e-brightgreen)
...
> **한 줄 요약** — 단순 기능 구현을 넘어 **테스트 자동화(유닛 151 + E2E 305) · 데이터 정합성 검증 ·
...
- **테스트 자동화 456개** — Vitest 유닛 151 + Playwright **E2E 305**(스모크·모드·문항유형·네비·설정·영속성·엣지·반응형·접근성·**상태 전이 전수**·**대용량 import**·**표/그림 문항**·**라이트박스/콘솔**·**저장 불가 환경**). 전체 시나리오: [`docs/e2e-test-scenarios.md`](docs/e2e-test-scenarios.md).
...
- **CI 품질 게이트** — GitHub Actions **11-job** 통과 시에만 머지: 기능·품질 8(lint·verify·**pdf-data(원본 PDF 대조)**·unit·**mutation**·build·e2e·**nonfunctional**) + **보안 3(의존성 감사·시크릿 스캔·CodeQL 정적분석)**. unit은 커버리지 임계값, mutation은 뮤테이션 스코어(break 85), build는 번들 크기 예산, nonfunctional은 성능·부하·메모리·타이머·오프라인·데이터 내구성까지 게이트. 추가로 **매일 예약 E2E**(`daily-e2e.yml`, KST 09:17)가 회귀를 상시 감시하고 실패 시 이슈로 알림.

## QA 역량 매핑

| QA 역량 | 본 프로젝트에서 한 일 | 근거 |
|---------|----------------------|------|
| 테스트 자동화 | Playwright **E2E 305개** + Vitest **유닛 151개**(속성 기반 21 포함) 작성·CI 연동 | `e2e/`, `src/**/*.test.ts` |
...
| 회귀 방지 | 결함 수정마다 회귀 테스트 추가, CI 머지 게이트 | 파서 회귀 케이스, 11-job CI |
...
```
     ...
     ───────  유닛 (Vitest) — 151개: 정답판정·파서·컷스코어·챕터/회차 통계·저장·타이머·토스트·store 액션·store↔storage 연동·속성 기반 21
...
- GitHub Actions **11 job**(push/PR 병렬):
  - **기능·품질 8** — `lint`(ESLint+tsc) · `verify(데이터)` · `pdf-data`(원본 PDF 대조 — 텍스트 2,489조각·정답 626·밑줄 146) · `unit`(+커버리지 임계값 게이트) · `mutation`(Stryker 뮤테이션 스코어 break 85 게이트) · `build`(+번들 크기 예산) · `e2e`(기능 305) · `nonfunctional`(성능·부하·메모리·타이머·오프라인·데이터 내구성·장기 스케일 12, CI 완화 예산).
...
- 자동화 테스트: **유닛 151 + E2E 305 = 456** (전 문항 626 렌더 스윕 별도).
...
- 번들: main JS **275KB(gzip 88KB)**, JS 합계 320.1KB, CSS 41KB(gzip 8KB). CI 번들 예산(JS 330KB·CSS 45KB)으로 회귀 감시.
...
flowchart LR
  ...
  subgraph 품질["품질 게이트 (GitHub Actions · 11 job)"]
    CI["기능·품질: lint · verify · pdf-data(PDF 대조) · unit(151) · mutation(93.2%) · build · e2e(305) · nonfunctional(12)<br/>보안: audit · secrets · codeql"]
  end
  REACT -.검증.-> CI
  SRC -.정합성 verify.-> CI
```
