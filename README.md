...
![tests](https://img.shields.io/badge/tests-617%20unit%20%2B%20423%20e2e-brightgreen)
...
> **한 줄 요약** — 단순 기능 구현을 넘어 **테스트 자동화(유닛 617 + E2E 423) · 데이터 정합성 검증 ·
...
- [테스트 전략](#테스트-전략)
- [테스트 자동화 상세 (E2E 423)](#테스트-자동화-상세-e2e-423)
...
- **테스트 자동화 1,040개** — Vitest 유닛 617 + Playwright **E2E 423**(기능 390 · 비기능 13 · **APK/WebView 20**). 스모크·모드·문항유형·네비·설정·영속성·엣지·반응형·접근성·**5모드 전이 25칸 전수**·**대용량 import**·**표/그림 문항**·**라이트박스/콘솔**·**저장 불가 환경**·**오프라인 출제**. 전체 시나리오: [`docs/e2e-test-scenarios.md`](docs/e2e-test-scenarios.md).
...
|---------|----------------------|------|
| 테스트 자동화 | Playwright **E2E 423개**(기능·비기능·APK) + Vitest **유닛 617개**(속성 기반 21 포함) 작성·CI 연동 | `e2e/`, `src/**/*.test.ts` |
...
- **연습 · 4지선다 · 시험 · 오답 · 퀵** 5가지 모드
- 연습·오답·퀵 모드: 답 선택 후 즉시 정답/해설 확인 / 시험·4지선다 모드: 채점 후 결과 확인
- **4지선다** — 지금 고른 세트에서 **보기가 4개인 문항만** 골라 섞어 냅니다(진위형·서답형·5지선다 제외 — 12세트에서 각각 15~50문항). 채점하면 시험과 같이 정답이 공개되고 **회차로 기록**되며, 틀린 문항은 오답 노트·오답 모드로 이어집니다. 다만 **합격 가늠(요약의 응시 횟수·평균·최고 정답률)에는 넣지 않습니다** — CSTS 본시험 세트에서 4지선다 50문항이 100점 중 정확히 75점(=합격선)만 덮어, 여기서 90%를 받아도 실제 배점으로는 67.5점(불합격)이기 때문입니다. 회차는 **세트별 회차 이력**에 모드 라벨과 함께 남습니다.
- **퀵** — 제품(ISTQB/CSTS)의 **전 세트를 통째로 섞어 한 문항씩 무한으로** 푸는 모드. 풀면 바로 정답·해설이 열리고 '다음 문제'로 넘어갑니다. 진행·정답·오답·연속 정답을 상시 보여주며 **제한시간도 기록도 없습니다**(회차 이력·오답 노트·챕터 통계 어디에도 남지 않습니다). 여러 세트에 재수록된 동일 문항은 **46그룹 96문항**을 빌드 타임에 식별해 한 바퀴에 두 번 나오지 않게 하고, 재수록본끼리 챕터명이 갈리는 3문항은 대표 챕터로 통일해 집계합니다.
  ...
  - **오답도 남지 않습니다** — 채점이라는 단계가 없어(한 문항씩 즉시 확인) 오답 노트·오답 모드 출제 대상·챕터별 정답률 어디에도 들어가지 않습니다. 틀린 문항은 그 자리에서 해설로 확인하고, 기록으로 남기려면 **시험** 모드에서 채점하세요.
...
```
        ▲  E2E (Playwright) — 423개: 사용자 플로우·엣지·크로스뷰포트·A11y(axe)·시드 랜덤 스모크·몽키·APK WebView
       ───
      ─────  통합/렌더 — jsdom 렌더 테스트(파서·RichText)
     ───────  유닛 (Vitest) — 617개: 정답판정·파서·컷스코어·챕터/회차 통계·저장·타이머·토스트·store 액션·store↔storage 연동·추첨 분포·속성 기반 21
...
- 자동화 테스트: **유닛 617 + E2E 423 = 1,040** (E2E 내역: 기능 390 · 비기능 13 · APK/WebView 20. 전 문항 626 렌더 스윕 포함).
- 수치는 2026-08-06 실측이다. 스위트가 늘면 어긋나므로 **정본은 CI 로그**다 — 판단 근거로 쓰지 않는다.
- 뮤테이션 스코어(채점·통계·저장키·모드규칙 6개 유틸): **91.9%**(555개 뮤턴트 중 510 kill) — CI break 85 게이트로 테스트 효력 저하를 차단.
- 유닛 커버리지(로직 계층 `store`·`utils`·`hooks`): **Stmts 78.6% · Branch 72.7% · Funcs 77.3% · Lines 80.3%**. CI에서 임계값(stmt 75·branch 71·func 74·line 77) 게이트로 회귀 차단 — 실측보다 약 2%p 낮게 잡은 **래칫**이라 올릴 때마다 임계값도 함께 올린다. 컴포넌트·앱 셸은 뷰 계층이라 E2E가 담당한다.
- 데이터 무결성: 626문항 정답·이미지·스키마 `verify` 통과 + **원본 PDF 대조 게이트**(텍스트 2,489조각·정답 626/626·밑줄 146/146 — 불일치 0), 전수 스윕 결과 **HTTP 404·예외·깨진 이미지 0**.
- 번들(gzip 기준): JS **120.6KB / 예산 140KB(86%)** · CSS **8.9KB / 예산 12KB(74%)**. CI `build` job의 `npm run size`로 회귀 감시. PWA precache 110 엔트리(문항 JSON 전 세트 포함 — 오프라인 퀵 출제가 여기 의존).
