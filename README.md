...
![tests](https://img.shields.io/badge/tests-525%20unit%20%2B%20436%20e2e-brightgreen)
...
> **한 줄 요약** — 단순 기능 구현을 넘어 **테스트 자동화(유닛 525 + E2E 436) · 데이터 정합성 검증 ·
...
- [테스트 전략](#테스트-전략)
- [테스트 자동화 상세 (E2E 436)](#테스트-자동화-상세-e2e-436)
...
- **테스트 자동화 961개** — Vitest 유닛 525 + Playwright **E2E 436**(기능 403 · 비기능 13 · **APK/WebView 20**). 스모크·모드·문항유형·네비·설정·영속성·엣지·반응형·접근성·**5모드 전이 25칸 전수**·**대용량 import**·**표/그림 문항**·**라이트박스/콘솔**·**저장 불가 환경**·**오프라인 출제**. 전체 시나리오: [`docs/e2e-test-scenarios.md`](docs/e2e-test-scenarios.md).
...
|---------|----------------------|------|
| 테스트 자동화 | Playwright **E2E 436개**(기능·비기능·APK) + Vitest **유닛 525개**(속성 기반 22 포함) 작성·CI 연동 | `e2e/`, `src/**/*.test.ts` |
...
```
        ▲  E2E (Playwright) — 436개: 사용자 플로우·엣지·크로스뷰포트·A11y(axe)·시드 랜덤 스모크·몽키·APK WebView
       ───
...
- GitHub Actions **14 job**(push/PR 병렬):
  - **기능·품질 11** — `lint`(ESLint + `tsc` 앱 + `tsc -p tsconfig.test.json` 테스트·e2e) · `verify(데이터)` · `pdf-data`(원본 PDF 대조 — 텍스트 2,489조각·정답 626·밑줄 146) · `unit`(+커버리지 임계값 게이트) · `mutation`(Stryker 뮤테이션 스코어 break 85 게이트) · `build`(+번들 크기 예산) · `android-build`(Capacitor 동기화·APK 빌드) · `e2e`(기능 403) · `nonfunctional`(성능·부하·메모리·타이머·오프라인·데이터 내구성·장기 스케일 13, CI 완화 예산) · `apk`(Pixel 7 프로파일 + WebView UA + 안전영역 모사 20 — 실제 배포 형태인 APK의 회귀를 게이트로 감시) · `mutation-storage`(영속화·상태 계층 뮤테이션, break 50 래칫).
...
- 자동화 테스트: **유닛 525 + E2E 436 = 961** (E2E 내역: 기능 403 · 비기능 13 · APK/WebView 20. 전 문항 626 렌더 스윕 포함).
