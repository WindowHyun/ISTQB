...
- **오답 노트** — 채점 회차를 **세트별로 모아** 선택 → 그 세트의 **문제번호 · 내가 고른 답 · 정답** 표시, **전 회차 오답 합집합**으로 누적(Phase 3). 문항을 누르면 노트가 닫히고 지문·보기·해설이 **본문 화면**에 펼쳐지며(팝업 아님), `‹ N / M ›`로 같은 세트의 다른 오답으로 이동한다 — 퀵 오답과 같은 화면을 쓴다
...
```
    ...
    ─────────  데이터 검증 — verify + PDF 전수 대조 상시 게이트(텍스트 2,491·정답 626·밑줄 146)
```
...
- 데이터 무결성: 626문항 정답·이미지·스키마 `verify` 통과 + **원본 PDF 대조 게이트**(텍스트 2,491조각·정답 626/626·밑줄 146/146 — 불일치 0), 전수 스윕 결과 **HTTP 404·예외·깨진 이미지 0**.
