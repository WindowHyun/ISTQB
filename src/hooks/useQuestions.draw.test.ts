...
describe('퀵 추첨 — 세트 분포', () => {
   ...
   *
   * ROUNDS를 줄이지 말 것. 아래 ±10% 허용폭이 통계적 요동이 아니라 '진짜 편향'에만
   * 반응하려면 표본이 충분해야 한다. 세트 비중 p인 세트의 관측 비율은 상대표준편차
   * sqrt((1-p)/(N·p))를 가지므로, 허용폭 0.1이 5σ를 넘으려면 N > 2500·(1-p)/p 이다.
   * 실측 최소 비중은 CSTS 4.9%(7세트 390문항) → N > 48,816. 지금 N = 3,000 × 20 = 60,000이라
   * 여유가 크지 않다. 줄이면 시드를 바꾸는 순간 흔들리는 검사가 된다.
   *
   * timeout: 이 두 건이 유닛 스위트에서 가장 무겁다(CSTS 단독 570ms, 전체 병렬 실행 시
   * 1,019ms — 2위의 3배). 기본 5초로는 여유가 5배뿐이라, 같은 머신에서 E2E가 함께 돌면
   * CPU 경합만으로 초과했다(실제로 그렇게 한 번 실패했고 단독 실행 11회는 전부 통과했다).
   * 표본이 정당한 비용이므로 줄이는 대신 여유를 준다 — 진짜 무한 루프는 30초에 여전히 잡힌다.
   */
  it.each([['ISTQB'], ['CSTS']])('%s — 세트별 출현 비율이 풀 비율에서 크게 벗어나지 않는다', { timeout: 30_000 }, (cert) => {
