...
export const BACK_PRIORITY = {
  /**
   * 화면을 통째로 차지하는 것 — 시험 응시 중 이탈 가드, 오답 보기 전체화면.
   * 가장 아래다: 열린 오버레이가 모두 닫힌 뒤에야 걸린다.
   * (둘은 동시에 존재할 수 없다 — 오답 보기가 켜지면 워크스페이스가 언마운트된다.)
   */
  exam: 5,
  screen: 5,
  /** 모바일 드로어. 그 위에 열린 모달이 먼저 닫힌다. */
  drawer: 10,
  ...
  confirm: 30,
  /**
   * 이미지 라이트박스 — 모달 위에도 뜨는 유일한 오버레이라 가장 높다.
   * 종전에는 여기 등록되지 않은 채였다. 그래서 그림을 확대한 상태에서 안드로이드
   * 하드웨어 뒤로가기를 누르면 라이트박스가 아니라 **그 아래 모달이 닫히거나**
   * (모달 위에 겹친 경우) **앱을 벗어났다** — 이 모듈이 막으려던 바로 그 증상이다.
   * Esc·배경 탭은 동작했기 때문에 웹에서는 드러나지 않고 APK에서만 나타났다.
   */
  lightbox: 40,
} as const;
