...
/**
 * 세트 전체를 대상으로 한 '실전' 회차인지 — **회차 타임라인에 실을 것**의 기준이다.
 * 챕터 미니 시험(10문항)은 제외한다.
 */
...
/**
 * 합격 가늠(요약의 응시 횟수·평균·최고 정답률)에 넣을 회차인지.
 *
 * 타임라인과 갈라 놓은 이유는 **4지선다 하나** 때문이고, 근거는 배점이다.
 * CSTS 본시험 6세트는 4지선다 50문항이 정확히 100점 중 **75.0점**을 차지하는데,
 * 그 75점이 곧 CSTS 합격선이다. 그래서 4지선다에서 45/50을 맞히면 그 회차는 90%로
 * 기록되지만 실제 배점으로는 67.5점 — 불합격선 아래다. 요약은 '합격 기준 75점 이상'
 * 배너 **바로 아래**에 있으므로, 그 90%가 합격 신호로 읽힌다.
 *
 * 폐지된 랜덤 모드를 요약에 넣었던 것과는 성질이 다르다. 랜덤은 같은 모집단에서
 * 무작위로 뽑아 **편향 없이 표본만 작았지만**, 4지선다는 진위형·서답형을 통째로 빼는
 * **체계적 편향**이다 — 빠진 유형이 배점의 25%다. 그래서 선례가 여기엔 적용되지 않는다.
 *
 * 대신 타임라인에는 그대로 남긴다(isSetLevelRound). 거기서는 모드별로 갈라 델타·성장폭을
 * 계산하고 행마다 모드 이름이 붙어, 시험 회차와 섞여 읽힐 여지가 없다.
 * 두 술어가 갈리면서 "요약의 응시 횟수 ≠ 타임라인 행 수"가 되는데, 그 어긋남은 요약
 * 주석이 무엇을 세는지 밝혀 설명한다(예전 '요약 5 / 타임라인 3' 결함은 라벨 없이
 * 숫자만 달랐던 것이 문제였다).
 */
export function isPassGaugeRound(h: ExamHistory): boolean {
  return isSetLevelRound(h) && h.mode !== 'choice';
}
